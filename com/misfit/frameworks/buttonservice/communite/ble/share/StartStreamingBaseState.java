package com.misfit.frameworks.buttonservice.communite.ble.share;

import com.misfit.frameworks.buttonservice.communite.ble.BleAdapter;
import com.misfit.frameworks.buttonservice.communite.ble.BleSession;
import com.misfit.frameworks.buttonservice.communite.ble.BleState;
import com.misfit.frameworks.buttonservice.communite.ble.SdkCallback;
import com.misfit.frameworks.common.constants.MFNetworkReturnCode;

public abstract class StartStreamingBaseState extends BleState {
    private BleAdapter bleAdapter;
    private BleSession currentSession;
    private SdkCallback sdkCallback;

    protected abstract BleState getStateOnCompleted(boolean z);

    protected abstract BleState getStateOnTimeOut();

    public StartStreamingBaseState(SdkCallback sdkCallback, BleAdapter bleAdapter, BleSession bleSession) {
        super(bleSession.getClass().getSimpleName());
        this.bleAdapter = bleAdapter;
        this.currentSession = bleSession;
        this.sdkCallback = sdkCallback;
    }

    public boolean onEnter() {
        super.onEnter();
        startTimeout();
        if (this.bleAdapter.startStreaming(this.sdkCallback)) {
            return true;
        }
        stopTimeout();
        BleState stateOnCompleted = getStateOnCompleted(false);
        if (stateOnCompleted == null) {
            return false;
        }
        this.currentSession.enterStateWithDelayTime(stateOnCompleted, MFNetworkReturnCode.INTERNAL_SERVER_ERROR);
        return false;
    }

    protected void onTimeout() {
        super.onTimeout();
        this.currentSession.log("Start streaming timeout.");
        this.currentSession.enterState(getStateOnTimeOut());
    }

    public boolean handleOnStartStreamingCompleted(boolean z) {
        stopTimeout();
        this.currentSession.enterState(getStateOnCompleted(z));
        return true;
    }
}
