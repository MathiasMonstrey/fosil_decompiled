...
final class ce {
    ...
    static List<File> m6039a(Context context, ApplicationInfo applicationInfo, File file, boolean z) throws IOException {
        List<File> b;
        Log.i("MultiDex", "MultiDexExtractor.load(" + applicationInfo.sourceDir + ", " + z + ")");
        File file2 = new File(applicationInfo.sourceDir);
        long h = m6048h(file2);
        if (z || m6045a(context, file2, h)) {
            Log.i("MultiDex", "Detected that extraction must be performed.");
            b = m6046b(file2, file);
            m6041a(context, m6047g(file2), h, b.size() + 1);
        } else {
            try {
                b = m6040a(context, file2, file);
            } catch (Throwable e) {
    ...
    private static List<File> m6040a(Context context, File file, File file2) throws IOException {
        Log.i("MultiDex", "loading existing secondary dex files");
        String str = file.getName() + ".classes";
        int i = m6051k(context).getInt("dex.number", 1);
    ...
    private static boolean m6045a(Context context, File file, long j) {
        SharedPreferences k = m6051k(context);
        return (k.getLong("timestamp", -1) == m6047g(file) && k.getLong("crc", -1) == j) ? false : true;
    }

    private static long m6047g(File file) {
    ...
    private static long m6048h(File file) throws IOException {
        long h = cf.m6131h(file);
        if (h == -1) {
            return h - 1;
        }
        return h;
    ...
    private static List<File> m6046b(File file, File file2) throws IOException {
        String str = file.getName() + ".classes";
        m6043a(file2, str);
        ...
        try {
            ...
            while (entry != null) {
                ...
                while (i2 < 3 && !z) {
                    int i3 = i2 + 1;
                    m6044a(zipFile, entry, file3, str);
                    boolean j = m6050j(file3);
                    Log.i("MultiDex", "Extraction " + (j ? "success" : "failed") + " - length " + file3.getAbsolutePath() + ": " + file3.length());
                    if (!j) {
                        file3.delete();
                        if (file3.exists()) {
    ...
    private static void m6041a(Context context, long j, long j2, int i) {
        Editor edit = m6051k(context).edit();
    ...
    private static SharedPreferences m6051k(Context context) {
        return context.getSharedPreferences("multidex.version", VERSION.SDK_INT < 11 ? 0 : 4);
    }

    private static void m6043a(File file, final String str) throws IOException {
        m6049i(file.getParentFile());
        m6049i(file);
    ...
    private static void m6049i(File file) throws IOException {
        file.mkdir();
        if (!file.isDirectory()) {
    ...
    private static void m6044a(ZipFile zipFile, ZipEntry zipEntry, File file, String str) throws IOException, FileNotFoundException {
    ...
    static boolean m6050j(java.io.File r4) {
        /*
    ...
    L_0x004d:
        ...
        goto L_0x0027;
        */
        throw new UnsupportedOperationException("Method not decompiled: com.fossil.ce.j(java.io.File):boolean");
    }
    ...
    static {
        try {
            Fp = Editor.class.getMethod("apply", new Class[0]);
        } catch (NoSuchMethodException e) {
            Fp = null;
        }
    }

    private static void m6042a(Editor editor) {
        if (Fp != null) {
            try {
                Fp.invoke(editor, new Object[0]);
                return;
